...
import { Shield, ChevronLeft, CheckCircle2, Zap, Brain, Target, Sparkles } from 'lucide-react';
...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const levels: { id: UserPersona['familiarity'], label: string, desc: string, icon: React.ReactNode }[] = [
    { 
      id: 'expert', 
      label: 'מכיר/ה היטב', 
      desc: 'אני יודע/ת להסתדר ומזהה את רוב הטריקים.',
      icon: <Zap className="text-emerald-500" />
    },
    { 
      id: 'intermediate', 
      label: 'מכיר/ה קצת', 
      desc: 'יש לי קצת ניסיון אבל אני רוצה להיות בטוח/ה.',
      icon: <Target className="text-sky-500" />
    },
    { 
      id: 'beginner', 
      label: 'לא מכיר/ה', 
      desc: 'אני מרגיש/ה קצת אבוד/ה או מודאג/ת בגלישה ברשת.',
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-slate-800 text-right" dir="rtl">
      <div className="max-w-xl w-full space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        {step === 1 && (
          <div className="space-y-10 animate-in fade-in slide-in-from-left-4 duration-500">
            <div className="text-center space-y-4">
              ...
              <h1 className="text-4xl font-brand font-black tracking-tight text-slate-800">ברוכים הבאים למסע.</h1>
              <p className="text-slate-500 text-lg font-medium">כדי שנוכל להתאים את החוויה עבורכם, מהי קבוצת הגיל שלכם?</p>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-10 animate-in fade-in slide-in-from-left-4 duration-500">
            <div className="text-center space-y-4">
              <h1 className="text-4xl font-brand font-black tracking-tight text-slate-800">ניסיון דיגיטלי</h1>
              <p className="text-slate-500 text-lg font-medium">כמה בטוחים אתם מרגישים בגלישה באינטרנט?</p>
            </div>
